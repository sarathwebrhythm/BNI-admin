...
<div class="space-y-6">
    ...
    <!-- Header navigation -->
    <div>
        <a href="{{ route('admin.members.show', $member->id) }}" class="inline-flex items-center text-sm font-medium text-slate-400 hover:text-slate-200 transition">
    ...
    <div class="bg-slate-900 border border-slate-800/80 rounded-2xl shadow-lg p-8 max-w-3xl">
        <h3 class="text-lg font-semibold text-slate-200 mb-6 pb-3 border-b border-slate-850">Edit Member Information</h3>

        <form action="{{ route('admin.members.update', $member->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="company" name="company" value="{{ old('company', $member->company) }}"
                        class="mt-1.5 appearance-none block w-full px-3 py-2 border border-slate-850 rounded-xl bg-slate-950/60 text-white focus:outline-none focus:ring-2 focus:ring-brand focus:border-brand text-sm transition-all">
                </div>
                <!-- Joining Date -->
                <div>
                ...
                <div>
                    ...
                    <input
                        ...
                        id="expire_date"
                        name="expire_date"
                        value="{{ old('expire_date', optional($member->expire_date)->format('Y-m-d')) }}"
                        class="mt-1.5 appearance-none block w-full px-3 py-2 border border-slate-850 rounded-xl bg-slate-950/60 text-white focus:outline-none focus:ring-2 focus:ring-brand focus:border-brand text-sm transition-all">
                </div>
                <!-- Chapter -->
                <div>
                    <label for="chapter" class="block text-sm font-medium text-slate-300">BNI Chapter</label>
                    <input type="text" id="chapter" name="chapter" value="{{ old('chapter', $member->chapter) }}"
                        class="mt-1.5 appearance-none block w-full px-3 py-2 border border-slate-850 rounded-xl bg-slate-950/60 text-white focus:outline-none focus:ring-2 focus:ring-brand focus:border-brand text-sm transition-all">
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" id="designation" name="designation" value="{{ old('designation', $member->designation) }}"
                        class="mt-1.5 appearance-none block w-full px-3 py-2 border border-slate-850 rounded-xl bg-slate-950/60 text-white focus:outline-none focus:ring-2 focus:ring-brand focus:border-brand text-sm transition-all">
                </div>
            </div>
            ...
            <div class="pt-4 border-t border-slate-850 flex items-center justify-end space-x-3">
                <a href="{{ route('admin.members.show', $member->id) }}" class="px-4 py-2 border border-slate-800 hover:border-slate-700 text-slate-300 rounded-xl text-sm font-semibold transition">
                    Cancel
                </a>

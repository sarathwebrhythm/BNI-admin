...
<head>
    ...
    <title>Sign In - BNI Admin Portal</title>
    <!-- Google Fonts -->
...
<body class="h-full text-slate-100 flex flex-col justify-center py-12 sm:px-6 lg:px-8 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-slate-950 to-black">
    ...
    <div class="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <h2 class="mt-6 text-center text-3xl font-extrabold text-white tracking-tight">BNI Admin Portal</h2>
        <p class="mt-2 text-center text-sm text-slate-400">
            Sign in to manage members and reports.
        </p>
    </div>
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center">
                        <input id="remember" name="remember" type="checkbox"
                            class="h-4 w-4 text-brand focus:ring-brand border-slate-800 rounded bg-slate-950/60 cursor-pointer">
                        <label for="remember" class="ml-2 block text-sm text-slate-300 cursor-pointer select-none">Remember Me</label>
                    </div>

                    <div class="text-sm">
                        <a href="{{ route('admin.forgot-password') }}" class="font-medium text-brand-light hover:text-brand-light transition-colors">
                            Forgot Password?
                        </a>
                    </div>
                </div>
